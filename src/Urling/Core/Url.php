...
final class Url
{
    ...
    public function isSameOrigin(string $url, string $origin = null, bool $verify_protocols = true): bool
    {
        ...
        if (!LogicVerifier::verify(fn() => LogicVerifier::isNotNullAndNotEmpty($origin))) {
            return false;
        }
        ...
        if (
            !LogicVerifier::verify(fn() => LogicVerifier::isNotNullAndNotEmpty($url_hostname)) ||
            !LogicVerifier::verify(fn() => LogicVerifier::isNotNullAndNotEmpty($origin_hostname))
        ) {
            return false;
        }
